...
//	}
	...
	protected float Q = 8; //25; // complexity of the assumed distributions
...
//	}
	...
	public ImageProcessor srm2D(ImageProcessor ip, boolean showAverages) {
		...
		if (showAverages) {
			int[] pixColor=new int[w*h];
			for (int i = 0; i < w*h; i++)
				for(int j=0;j<3;++j)
				pixColor[i]+=((int)average[getRegionIndex(i)][j])<<(8*(2-j));
//				average[i] = average[getRegionIndex(i)];
			ip = new ColorProcessor(w, h,pixColor);
		}
//		else {
//			int regionCount = consolidateRegions();
//
...
//		String title = image.getTitle() + " (SRM Q=" + Q + ")";
	...
	void initializeRegions2D(int[] pixel, int w, int h) {
		average = new float[w * h][3];
		count = new int[w * h];
		regionIndex = new int[w * h];

		for (int i = 0; i < w*h; i++) {
			for(int j=0;j<3;++j){
				average[i][j] = (pixel[i]>>(8*(2-j)) & 0xff);
			}
			count[i] = 1;
			regionIndex[i] = i;
		}
...
//	}
	...
	protected void addNeighborPair(int neighborIndex,
			int[] pixel, int i1, int i2) {
		int difference = Math.abs((pixel[i1]>>16 & 0xff)
				- (pixel[i2]>>16 & 0xff));
		for(int i=1;i<3;++i){
			int tmpDiff=Math.abs((pixel[i1]>>(8*(2-i)) & 0xff) - (pixel[i2]>>(8*(2-i)) & 0xff));
	...
	void initializeNeighbors2D(int[] pixel, int w, int h) {
...
//	}
	...
	boolean predicate(int i1, int i2) {
		float difference=0;
		for(int i=0;i<3;++i){
			float tmpDiff= Math.abs(average[i1][i] - average[i2][i]);
			if(tmpDiff>difference){
				difference=tmpDiff;
			}
		}
	...
	void mergeRegions(int i1, int i2) {
		...
		float[] mergedAverage = new float[3];
		for(int i=0;i<3;++i){
			mergedAverage[i]=(average[i1][i] * count[i1]
				+ average[i2][i] * count[i2]) / mergedCount;
		}
	...
	Map<Integer,List<Integer>> regionPixels;
	public Map<Integer,List<Integer>> getRegionPixels(){
		regionPixels=new HashMap<Integer,List<Integer>>();
		for(int i=0;i<count.length;++i){
			int regionIdx=getRegionIndex(i);
			if(!regionPixels.containsKey(regionIdx)){
				List<Integer> region=new ArrayList<Integer>();
				region.add(i);
				regionPixels.put(regionIdx,region);
			}
			else{
				regionPixels.get(regionIdx).add(i);
			}
		}
